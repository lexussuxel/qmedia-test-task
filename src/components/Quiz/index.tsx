...
const Quiz: FC<QuizProps> = ({ question, next, prev, count, checked_num }) => {
  ...
  const onNextClickHandler = () => {
    if (checked === undefined) setError(true);
    else {
      next(checked);
    }
  };
  ...
  function circleProgress(): ReactNode[] {
    ...
    for (let i = 0; i < count; i++)
      if (i === question.id)
        circles.push(
          <circle cx={10 + i * 30} cy="10" r="10" fill="#00A5FF" key={i} />
        );
      else
        circles.push(
          <circle cx={10 + i * 30} cy="10" r="10" fill="#89D1F8" key={i} />
        );
    return circles;
  }
    ...
    <div className={styles.wrapper}>
      ...
      <div className={styles.buttons_wrapper}>
        {question.id !== 0 && (
          <button
            onClick={prev}
            className={styles.base_button + " " + styles.prev_button}
          >
            Назад
          </button>
        )}
        <button
          ...
          className={styles.base_button + " " + styles.next_button}
        >
          {question.id !== count - 1 ? "Дальше" : "Узнать результат"}
        </button>
      </div>
    </div>
  );
